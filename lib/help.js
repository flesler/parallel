...
exports.getText = () => {
  ...
  log('  parallel [options] [command [arguments]] ::: arguments')
  log('  cat/ls/etc ... | parallel [options] [command [arguments]]')
  log()
  log('Options:')
...
function format(key, placeholder, pad) {
  const padded = `  {${key}}`.padEnd(pad)
  return `${padded} ${placeholder.desc}`
}

function maxLength() {
  const placeholders = require('./placeholders').placeholders
  const lengths = Object.keys(placeholders).map(k => `{${k}}`.length)
