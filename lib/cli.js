...
const options = {
  ...
  'block': {
    desc: 'Split stdin by blocks of this size (e.g., 1M, 1K) when using --pipe',
    fn: function (size) {
      opts.block = parseSize(size)
      if (!opts.block) util.fatal(`Invalid block size: ${size}`)
    }
  },
  'dry-run': {
    alias: 'D',
