...
const placeholders = {
  ...
  '...': {
    handler: (val) => val.replace(/\.[^.]*\.[^.]*\.[^.]*$/, ''),
    desc: 'the input line without three extensions (e.g., file.tar.gz.backup → file)',
    gnu: true
  },
  '/..': {
    handler: (val) => placeholders['..'].handler(placeholders['/'].handler(val)),
  ...
  't': {
    handler: () => Date.now(),
    desc: 'current time as a number',
    gnu: false
  },
  'T': {
    handler: () => new Date().toISOString().replace(/\D+/g, '_').replace(/_$/, ''),
    desc: 'current time in ISO as a string',
    gnu: false
  },
  'd': {
    handler: () => new Date().toISOString().split('T')[0].replace(/\D+/g, '_'),
